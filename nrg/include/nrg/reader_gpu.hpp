...
namespace nrgprf
{
    ...
    public:
        ...
        explicit reader_gpu(readings_type::type, device_mask, std::ostream & = std::cout);
        ...
        result<units_power>
            get_board_power(const sample&, uint8_t) const noexcept;

        result<units_energy>
            get_board_energy(const sample&, uint8_t) const noexcept;
