...
// TODO: Need to confirm whether sensor GSIDs are constant or dynamically
// assigned (at reboot, for example)
...
namespace detail {
template <typename T> std::string to_hex_string(const T &arg) {
...
template <size_t sz> size_t retrieve_field(char *into, const void *from) {
  if constexpr (sz > 1)
    std::memcpy(into, from, sz);
  else
...
bool assert_header_block(const sensor_data_header_block &hb, std::string &msg) {
  ...
  if (!hb.valid)
    msg = cmmn::concat(
        "Header block not valid; static data not ready for consumption");
    ...
    msg = cmmn::concat("Incorrect ping buffer offset, expected 0x",
                       detail::to_hex_string(sensor_ping_buffer_offset),
                       ", found 0x",
    ...
    msg = cmmn::concat("Incorrect pong buffer offset, expected 0x",
                       detail::to_hex_string(sensor_pong_buffer_offset),
                       ", found 0x",
...
sensor_structure_v1_sample get_v1_sample(const sensor_readings_buffer &buffer,
                                         size_t offset) {
  assert(buffer.valid);
  ...
  const uint8_t *curr = buffer.readings + offset - sensor_readings_buffer::pad +
                        sizeof(sensor_structure_v1::gsid);
...
sensor_structure_v1
get_sensor_structure_v1(const sensor_readings_buffer &buffer, size_t offset) {
  assert(buffer.valid);
  using namespace detail;
...
sensor_structure_v2
get_sensor_structure_v2(const sensor_readings_buffer &buffer, size_t offset) {
  assert(buffer.valid);
  using namespace detail;
...
bool get_sensor_structure(const sensor_readings_buffer &buffer,
...
bool assert_sensor_structure(const sensor_structure &sstruct,
  ...
  case 1:
    ...
    if (!result)
      msg = cmmn::concat(
          "Sensor GSID are different between readings and names entry,"
          " found ",
          std::to_string(sstruct.sv1.gsid), " and ",
          std::to_string(nentry.gsid));
    return result;
  case 2:
    ...
    if (!result)
      msg = cmmn::concat(
          "Sensor GSID are different between readings and names entry,"
          " found ",
          std::to_string(sstruct.sv2.gsid), " and ",
          std::to_string(nentry.gsid));
    return result;
  }
  assert(false);
...
uint64_t get_timestamp(const sensor_readings_buffer &buffer, size_t offset) {
  assert(buffer.valid);
  uint64_t timestamp;
  const uint8_t *curr = buffer.readings + offset - sensor_readings_buffer::pad;
...
bool get_sensor_record(const sensor_buffers &buffs,
                       const sensor_names_entry &entry, sensor_structure &out) {
...
bool debug_print(const sensor_names_entry &entry,
                 const sensor_structure &record) {
  std::cout << entry << "\n";
  switch (entry.structure_version) {
  case 1:
    std::cout << "  " << record.sv1 << "\n";
...
template <typename T> constexpr uint16_t to_sensor_gsid() {
  using namespace nrgprf;
  static_assert(std::is_same_v<T, loc::sys> || std::is_same_v<T, loc::pkg> ||
                    std::is_same_v<T, loc::cores> ||
                    std::is_same_v<T, loc::uncore> ||
                    std::is_same_v<T, loc::mem> || std::is_same_v<T, loc::gpu>,
...
nrgprf::watts<double> canonicalize_power(uint16_t value,
                                         const occ::sensor_names_entry &entry) {
...
// this means that each value incremented in the counter corresponds to 1000/512
// ns
...
std::error_code get_sensor_buffers(std::ifstream &ifs, uint32_t occ_num,
...
std::error_code get_header(std::ifstream &ifs, uint32_t occ_num,
...
std::error_code
get_names_entries(std::ifstream &ifs, uint32_t occ_num,
...
std::error_code
get_sensor_structs(const occ::sensor_buffers &buffs,
...
std::istream &operator>>(std::istream &is, sensor_buffers &buffs) {
  return is.read(reinterpret_cast<std::istream::char_type *>(&buffs),
                 sizeof(buffs));
}
...
reader_impl::reader_impl(location_mask lmask, socket_mask smask,
                         std::ostream &os)
    : _file(std::make_shared<std::ifstream>(occ::sensors_file,
  ...
  os << fileline(
      cmmn::concat("Found ", std::to_string(*sockets), " sockets\n"));
  for (uint32_t occ_num = 0; occ_num < *sockets; occ_num++) {
    if (!smask[occ_num])
      continue;
    os << fileline(
        cmmn::concat("Registered socket: ", std::to_string(occ_num), "\n"));

    occ::sensor_data_header_block hb{};
    if (auto ec = get_header(*_file, occ_num, hb))
      throw exception(ec);

    std::vector<occ::sensor_names_entry> entries(hb.sensor_count,
                                                 occ::sensor_names_entry{});
...
std::error_code
reader_impl::add_event(const std::vector<occ::sensor_names_entry> &entries,
                       uint32_t occ_num, uint32_t loc, std::ostream &os) {
...
bool reader_impl::read_single_occ(const event_data &ed, sensor_buffers &sbuffs,
                                  sample &s, std::error_code &ec) const {
  ec == get_sensor_buffers(*_file, ed.occ_num, sbuffs);
  if (ec)
    return false;
  for (const auto &entry : ed.entries) {
    size_t stride =
        ed.occ_num * nrgprf::max_domains + sensor_gsid_to_index(entry.gsid);
...
// Since sensors are read in bulk, reading with an index reads all sensors in
// some OCC
