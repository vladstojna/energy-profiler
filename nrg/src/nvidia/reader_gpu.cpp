...
namespace nrgprf
{
    ...
    result<readings_type::type> reader_gpu_impl::support(nvmlDevice_t handle) noexcept
    {
        assert(handle);
    ...
    bool reader_gpu_impl::read_energy(
        sample& s, size_t stride, nvmlDevice_t handle, std::error_code& ec) noexcept
    {
        unsigned long long energy;
    ...
    result<units_energy> reader_gpu_impl::get_board_energy(const sample& s, uint8_t dev) const noexcept
    {
        return get_value<
