...
nrgprf::joules<double>
total_energy(const nrgprf::reader_rapl &reader, const nrgprf::sample &first,
...
nrgprf::joules<double> calibrate_busy_wait(const nrgprf::reader_rapl &reader,
...
// Samples the sensors and then loops until the value read changes, which
// implies that the sensor has refreshed. Returns the last sample read and the
// number of iterations the sensor took to refresh.
...
int main(int argc, char **argv) {
  try {
    ...
    std::cout << "Total energy: " << consumed.count() << " J\n";
    std::cout << "Wait subtracted: " << (consumed - calibrated_val * ia).count()
              << " J\n";
