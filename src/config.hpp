...
class bounds_t {
public:
  using position_range_t = std::pair<position_t, position_t>;
...
private:
  using holder_type = std::variant<std::monostate, address_range_t,
                                   position_range_t, function_t>;
  holder_type _value;
};
...
private:
  using holder_type =
      std::variant<std::monostate, method_total_t, method_profile_t>;
  holder_type _value;
};
