...
long tep::set_trap(long word) { return word; }

#endif // __x86_64__ || __i386__

#if __GLIBC__ == 2 && __GLIBC_MINOR__ < 32

const char *tep::sig_str(int signal) { return strsignal(signal); }

#else

const char *tep::sig_str(int signal) { return sigabbrev_np(signal); }
...
bool tep::is_child_event(int wait_status) {
  return is_clone_event(wait_status) || is_vfork_event(wait_status) ||
...
bool tep::is_breakpoint_trap(int wait_status) {
  return WIFSTOPPED(wait_status) && !(WSTOPSIG(wait_status) & 0x80) &&
         (WSTOPSIG(wait_status) == SIGTRAP);
}

bool tep::is_syscall_trap(int wait_status) {
  return WIFSTOPPED(wait_status) && WSTOPSIG(wait_status) == (SIGTRAP | 0x80);
}
