...
namespace
{
    std::string address_to_hex_string(uintptr_t addr)
    {
        ...
        auto [ptr, err] = std::to_chars(
            std::begin(str) + 2, std::end(str), addr, 16);
...
namespace tep
{
    bool address::is_function_call() const noexcept
    {
        return false;
    }

    bool function_call::is_function_call() const noexcept
    {
        return true;
    }
    ...
    {
        return cmmn::concat("address:",
            address_to_hex_string(x.value));
    }

    std::string to_string(const function_call& x)
    {
        return cmmn::concat("function_call:",
            address_to_hex_string(x.value));
    }

    std::string to_string(const function_return& x)
    {
        return cmmn::concat("function_return:",
            address_to_hex_string(x.value));
    }

    std::string to_string(const inline_function& x)
    {
        return cmmn::concat("inline_function:",
            address_to_hex_string(x.value));
    }

    std::string to_string(const source_line& x)
    {
        return cmmn::concat("source_line:",
            address_to_hex_string(x.value));
    }
