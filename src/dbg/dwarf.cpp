...
namespace {
bool operator<(const tep::dbg::source_location &lhs,
               const tep::dbg::source_location &rhs) noexcept {
...
std::vector<Dwarf_Die> get_funcs(Dwarf_Die &cu_die) {
  ...
  ptrdiff_t res = dwarf_getfuncs(
      &cu_die,
      [](Dwarf_Die *func_die, void *data) {
...
size_t get_inline_instance_count(Dwarf_Die &func_die) {
  ...
  using tep::dbg::exception;
  size_t count = 0;
  int res = dwarf_func_inline_instances(
      &func_die,
      [](Dwarf_Die *, void *data) {
...
std::vector<tep::dbg::contiguous_range> get_ranges(Dwarf_Die &die) {
  ...
  ptrdiff_t offset = 0;
  while (
      (offset = dwarf_ranges(&die, offset, &base, &rng.low_pc, &rng.high_pc))) {
    if (offset == -1)
      throw exception(dwarf_errno(), dwarf_category());
    rngs.push_back(rng);
  }
...
source_location::source_location(call_param x) {
  ...
  if (dwarf_hasattr_integrate(&inst, DW_AT_call_file)) {
    if (0 != dwarf_formudata(
                 dwarf_attr_integrate(&inst, DW_AT_call_file, &attr), &val))
      throw exception(dwarf_errno(), dwarf_category());
    file = dwarf_filesrc(files, val, nullptr, nullptr);
  }
  if (dwarf_hasattr_integrate(&inst, DW_AT_call_line)) {
    if (0 != dwarf_formudata(
                 dwarf_attr_integrate(&inst, DW_AT_call_line, &attr), &val))
      throw exception(dwarf_errno(), dwarf_category());
  ...
  if (dwarf_hasattr_integrate(&inst, DW_AT_call_column)) {
    if (0 != dwarf_formudata(
                 dwarf_attr_integrate(&inst, DW_AT_call_column, &attr), &val))
      throw exception(dwarf_errno(), dwarf_category());
    if (val >= std::numeric_limits<uint32_t>::max())
      throw exception(errc::line_column_overflow);
    else
...
inline_instance::inline_instance(const param &x)
    : call_loc(std::in_place, x), addresses(x) {
  auto &inst = x.func_die;
...
compilation_unit::compilation_unit(const param &x)
    : path(build_path(x.cu_die)), addresses(get_ranges(x.cu_die)) {
...
void compilation_unit::load_lines(const param &x) {
  ...
  std::sort(lines.begin(), lines.end(),
            [](const source_line &lhs, const source_line &rhs) {
              if (lhs.file < rhs.file)
                return true;
              if (lhs.file == rhs.file) {
...
void compilation_unit::load_functions(const param &x) {
  static auto pred = [](const function &lhs, const function &rhs) {
    return lhs.die_name == rhs.die_name && lhs.decl_loc == rhs.decl_loc &&
  ...
  for (auto &func_die : get_funcs(x.cu_die)) {
    ...
    bool is_concrete = dwarf_hasattr(&func_die, DW_AT_low_pc) ||
  ...
  auto separator = std::partition(
      inlined.begin(), inlined.end(), [this](const function &inl) {
        return funcs.end() ==
               std::find_if(funcs.begin(), funcs.end(),
                            [&](const function &x) { return pred(x, inl); });
      });
...
inline_instances::inline_instances(const param &x) : insts(get_instances(x)) {}

std::vector<inline_instance> inline_instances::get_instances(const param &x) {
...
function_addresses::function_addresses(const param &x)
  ...
  assert(dwarf_hasattr(&x.func_die, DW_AT_low_pc) ||
...
void function::set_out_of_line_addresses(function_addresses x,
                                         passkey<compilation_unit>) {
  addresses = std::move(x);
}

void function::set_inline_instances(inline_instances x,
                                    passkey<compilation_unit>) {
  instances = std::move(x);
}

bool function::is_static() const noexcept { return !is_extern(); }

bool function::is_extern() const noexcept { return bool(linkage_name); }

bool operator==(const source_location &lhs,
                const source_location &rhs) noexcept {
  return lhs.file == rhs.file && lhs.line_number == rhs.line_number &&
