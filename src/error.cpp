...
tracer_error::tracer_error(tracer_errcode code) : _code(code), _msg() {}
...
tracer_errcode tracer_error::code() const { return _code; }
...
tracer_error::operator bool() const { return _code != tracer_errcode::SUCCESS; }
...
tracer_error tep::get_syserror__(const char *file, int line, int errnum,
  ...
  log::write(log::error, {"[%d] %s: %s", tid, comment, msg}, {file, line});
  return {code, msg};
}
